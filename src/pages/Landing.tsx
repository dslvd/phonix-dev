...
export default function Landing({ navigate, resetAppState }: LandingProps) {
  ...
  return (
    ...
    <div className="theme-page relative flex h-screen items-center justify-center overflow-hidden p-4 text-slate-100">
      ...
      {/* Login Card */}
      <Card className="max-w-md w-full animate-pop ml-auto mr-6">
        ...
        >
          ...
          {import.meta.env.DEV && (
            <div className="theme-surface-soft mb-4 rounded-xl border p-3 text-xs">
              <p className="mb-1 font-bold text-[#56b8e8]">Google Sign-In Setup (Development)</p>
              <p className="theme-muted">
                If you see a 403 error, add <code className="rounded bg-[color:var(--theme-bg-solid)] px-1">http://localhost:3000</code> to authorized origins in{' '}
                <a 
              ...
              <p className="theme-muted mt-1">Or just use <strong>Guest Login</strong> below.</p>
            </div>
          )}
          
          {/* Manual Google Button (fallback if no Client ID) */}
          {!clientId && (
            <div className="theme-surface-soft mb-4 cursor-pointer rounded-2xl border p-3 text-center transition-all hover:border-[#56b8e8]">
          ...
          <div className="flex items-center my-4">
            ...
            <span className="theme-muted px-4 text-sm font-semibold">or</span>
            <div className="flex-1 border-t border-[color:var(--theme-border)]"></div>
          </div>
